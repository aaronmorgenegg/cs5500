...
int getJob(){
	return rand.randint()%MAX_WORK_SIZE+1;
}
...
void receiveNewJobs(int new_job, int my_request, int job_flag, int my_status, int world_rank, std::vector<int> &job_queue){
	MPI_Irecv(&new_job,1,MPI_INT,ANY,JOB,MCW,&my_request);
	MPI_Test(&my_request,&job_flag,&my_status);
	if(!job_flag) return;
	job_queue.push_back(new_job);
	std::cout<<"p"<<world_rank<<": received job "<<new_job<<std::endl;
}

void doWork(std::vector<int> &job_queue){
	if(job_queue.size() > 0){
		usleep(job_queue[0]);
...
void distributeWork(std::vector<int> &job_queue, int world_size){
	if(job_queue.size()>MAX_JOB_QUEUE_SIZE){
		int job1 = job_queue.back();
		job_queue.pop_back();
		int job2 = job_queue.back();
		job_queue.pop_back();
		int dest1 = rand.randint()%world_size;
		int dest2 = rand.randint()%world_size;
...
void loadBalance(int world_rank, int world_size){
	...
	while(1){
		receiveNewJobs(new_job, my_request, job_flag, my_status, world_rank);
		distributeWork(job_queue, world_size);
		doWork(job_queue);
		// generateNewWork();
	}
}
